<template>
  ...
  <div v-if="product" class="flex justify-center items-center w-full">
    <div class="max-w-6xl min-h-screen py-10 px-4">
      <div
        class="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 bg-gradient-to-br from-pink-100 via-white to-purple-100 rounded-2xl shadow-lg p-6 border-2 border-pink-200">
        <!-- 🖼 商品圖片 -->
        <div class="relative w-full h-[400px] overflow-hidden group rounded-xl bg-white" @mousemove="handleMouseMove"
          @mouseleave="zoomVisible = false" ref="zoomBox">
          <img :src="imageUrl"
            class="w-full h-full object-contain transition-transform duration-300 group-hover:scale-105" />
          <div v-if="zoomVisible"
            class="absolute w-48 h-48 border-2 border-pink-400 rounded-lg pointer-events-none z-50" :style="lensStyle">
          </div>
        </div>
        ...
        <div class="space-y-5 flex flex-col justify-between">
          <div>
            <div class="flex justify-between items-center">
              ...
              <button @click="toggleFavorite(product.productId)"
                class="text-pink-400 hover:text-pink-600 text-2xl transition">
                <span v-if="favoriteStore.isInFavorites(product.productId)">❤️</span>
                <span v-else>🤍</span>
              </button>
            </div>

            <p class="text-xl font-semibold text-purple-700 mt-1">NT$ {{ product.price }}</p>
            <p :class="product.stockQuantity > 0 ? 'text-green-600' : 'text-red-500'" class="text-sm font-medium">
            ...
            <div class="flex items-center gap-3 mt-4">
              ...
              <div class="flex items-center border rounded-md overflow-hidden">
                <button @click="quantity = Math.max(1, quantity - 1)"
                  class="w-8 h-8 bg-pink-200 text-pink-700 hover:bg-pink-300">-</button>
                <input type="number" v-model.number="quantity" min="1" :max="product.stockQuantity"
                  class="w-12 text-center focus:outline-none" />
                <button @click="quantity = Math.min(product.stockQuantity, quantity + 1)"
                  class="w-8 h-8 bg-pink-200 text-pink-700 hover:bg-pink-300">+</button>
              </div>
            </div>
          </div>
      ...
      <div class="max-w-6xl mx-auto mt-10">
        ...
        <Swiper :slides-per-view="1.5" :space-between="16" grab-cursor :breakpoints="{
          ...
          <SwiperSlide v-for="item in recommended" :key="item.productId" class="!w-60">
            <div @click="goToProduct(item.productId)"
              ...
              <div class="relative z-10">
                <img :src="`${apiUrl}/images/${item.productId}`" alt="推薦商品"
...
function toggleFavorite(productId) {
  if (favoriteIdSet.value.has(productId)) {
    favoriteIdSet.value.delete(productId);
    // 可呼叫 axios.delete(`/favorites/${productId}`)
  } else {
